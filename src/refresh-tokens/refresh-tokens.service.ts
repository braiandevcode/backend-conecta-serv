...
export class RefreshTokenService {
  ...
  async revokeByToken(tokenDto: FindOneByTokenDto):Promise<void> {
    ...
    try {
      ...
      if(deleteToken.affected === 0){
        throw ErrorManager.createSignatureError(`BAD_REQUEST${ESeparatorsMsgErrors.SEPARATOR}No se encontró token para revocar`)
      }
    } catch (error) {
      const err = error as HttpException;
      if (err instanceof ErrorManager) throw err;
  ...
  async revokeAllByUser(idUser: string):  Promise<void>{
    try {
      ...
      // SI NO HUBO AFECTADOS
      if(deleteToken.affected === 0){
        throw ErrorManager.createSignatureError(`BAD_REQUEST${ESeparatorsMsgErrors.SEPARATOR}No se encontró usuario asociado a los registros de tokens`)
      }
